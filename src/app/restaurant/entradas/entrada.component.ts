...
export class EntradaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    x.snapshotChanges().subscribe(item => {
      ...
      this.menuList = this.menuList.filter( x => x.codigoCategoria === '100');
    });
  ...
  onDelete(key: string) {
    if (confirm('Are you sure to delete this Entrada ?') === true) {
      this.menuService.deletemenu(key);
      this.tostr.warning('Deleted Successfully', 'Entrada register');
    }
  }

  onSubmit(menuForm: NgForm) {
    this.menu = menuForm.value;
    this.menu.codigoCategoria = '100';
    this.menu.categoria = 'Entradas';
    this.menu.descripcion = 'entrada';
    ...
    this.tostr.success('Submitted Succcessfully', 'Entrada Register');
  }
